 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sherpa_api extends external_api {
    /**
    ...
    public static function save_placement(array $placement): array {
        ...
        $placementsave->type = $placementdata['name'];
        $placementsave->description = $placementdata['description'];
        $placementsave->pathmatch = $placementdata['pathmatch'];
        // Enable by default if not specified or empty.
        $placementsave->enabled =
            (empty($placementdata['enabled']) || $placementdata['enabled'] === 'true' || $placementdata['enabled'] === '1') ? 1 : 0;
        $placementsave->sortorder = is_numeric($placementdata['sortorder']) ? (int) $placementdata['sortorder'] : 0;

        // Check if this is a custom tour.
        $iscustom = isset($placementdata['custom']) && $placementdata['custom'];
        ...
        $placementsave->configdata = json_encode($configdata);
        ...
        if ($iscustom) {
            ...
            $placementsave->steps = $placementdata['steps'];

            $customsave = new \stdClass();
            $customsave->rawdata = json_encode($toursave);
            $customsave->courseid = $courseid;
            if (isset($placementdata['placementid']) && strlen($placementdata['placementid']) > 0) {
                $customsave->placementid = $placementdata['placementid'];
            }
        ...
        // Insert steps
        foreach ($placementdata['steps'] as $index => $step) {
    ...
    public static function create_tour_from_custom(int $courseid): array
    {
        ...
        // Decode the raw data
        $placementdata = json_decode($customtour->rawdata, true);
        if (!$placementdata) {
            return [
                'success' => false,
                'tourid' => 0,
        ...
        $coretour->name = $placementdata['name'] ?? 'Teacher Tour';
        $coretour->description = $placementdata['description'] ?? '';
        $coretour->pathmatch = $placementdata['pathmatch'] ?? '/course/view.php%id=' . $params['courseid'];
        $coretour->enabled = $placementdata['enabled'] ?? 1;
        $coretour->sortorder = isset($placementdata['sortorder']) ? (int) $placementdata['sortorder'] : 0;
    ...
    public static function get_pending_tours(int $courseid, string $shownids = '[]'): array
    {
        ...
        foreach ($customtours as $customtour) {
            ...
            $placementdata = json_decode($customtour->rawdata, true) ?? [];
            $sortorder = isset($placementdata['sortorder']) ? (int) $placementdata['sortorder'] : 1000;
            $name = $placementdata['name'] ?? 'Custom Tour';
    ...
    public static function start_custom_tour(int $customtourid): array
    {
        ...
        // Decode the raw data
        $placementdata = json_decode($customtour->rawdata, true);
        if (!$placementdata || empty($placementdata['steps'])) {
            return [
                'success' => false,
                'tourconfig' => '',
        ...
        $stepcount = count($placementdata['steps']);
        if ($stepcount == 1) {
        ...
        $tourconfig = [
            'name' => $placementdata['name'] ?? 'Teacher Tour',
            'startStep' => 0,
    ...
    public static function toggle_custom_tour_enabled(int $tourid, bool $enabled): array
    {
        ...
        $placementdata['enabled'] = $params['enabled'];
        $customtour->rawdata = json_encode($placementdata);
